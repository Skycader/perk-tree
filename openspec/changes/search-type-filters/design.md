...
- **Filter map, not four flags.** `_filters = { ability, combo, note, tip }`, keyed directly by entry `type` — one checkbox per type. (Wiki articles briefly shared the Заметки checkbox; split out because they're different content from notes.)
...
- **Last checked box is disabled** (`syncFilterLock()`), with the explanation as a `title` on its `<label>` (a disabled input doesn't reliably show its own tooltip). An all-unchecked state is unreachable, so it needs no dedicated handling — the empty "nothing found" message remains only for queries with no matches.
